-- Fix ambiguous column reference in create_consent_by_token function
CREATE OR REPLACE FUNCTION public.create_consent_by_token(patient_token text, consent_type_param text, content_param text)
...
BEGIN
  ...
  RETURNING informed_consents.id INTO new_consent_id;
  
  -- Return the created consent with qualified column names
  RETURN QUERY
  SELECT 
    ic.id::uuid,
    ic.patient_id::uuid,
    ic.consent_type::text,
    ic.content::text,
    ic.accepted::boolean,
    ic.signature_data::text,
    ic.accepted_at::timestamp with time zone,
    ic.created_at::timestamp with time zone
